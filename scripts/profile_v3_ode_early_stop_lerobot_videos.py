...
from scripts.profile_v3_ode_early_stop_lerobot_images import (
    CASE_SPECS,
    action_error,
    case_runtime_kwargs,
    pct_delta,
...
def append_stage_table(report, summaries):
    ...
    for key in priority + all_keys:
        ...
        if all(v == 0.0 for v in vals):
            continue
        report.append(
            f"| `{key}` | {vals[0]:.3f} | {vals[1]:.3f} | {vals[2]:.3f} | {pct_delta(vals[2], vals[0]):+.2f}% |"
        )
    report.append("")
...
def write_outputs(args, records, counts):
    ...
    for fixed_rec, safe_rec, trade_rec in zip(records["fixed_10"], records["early_safe"], records["early_tradeoff"]):
        ...
        report.append(
            ...
            f"{safe_errors[idx]['action_mae']:.6f} | {trade_errors[idx]['action_mae']:.6f} |"
        )
...
def main():
    ...
    parser.add_argument(
        "--report-path",
        default="/root/autodl-tmp/wall_x/workspace/vispruner_logs/libero_50video_v3_ode_early_stop_report.md",
    )
    parser.add_argument(
        "--results-json",
        default="/root/autodl-tmp/wall_x/workspace/vispruner_logs/libero_50video_v3_ode_early_stop_results.json",
    )
    ...
    print(f"[V3_VIDEO] report={args.report_path}", flush=True)
    print(f"[V3_VIDEO] results_json={args.results_json}", flush=True)

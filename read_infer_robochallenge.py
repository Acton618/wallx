...
# 阅读版说明：
# 这个文件是 scripts/infer_robochallenge.py 的注释副本，用来帮助理解
# RoboChallenge 在线推理链路。核心流水线是：
#   外部 state/views/instruction
#   -> preprocess: 组装机器人状态和相机观测
#   -> _construct_input: CPU 侧构造多模态输入
#   -> move_to_cuda: 把输入搬到 GPU
#   -> generate_flow_action / generate_ar_action: 模型动作预测
#   -> model_output_process/postprocess: 动作坐标转换和输出格式整理
#
# 注意：这里的注释只解释代码，不应作为生产入口；原始入口仍在
# wall_x/scripts/infer_robochallenge.py。
...
# 模型内部支持的动作自由度配置。这里的 key 会参与 normalizer、
# mask 构造和动作维度切片，因此它定义了机器人动作向量的语义布局。
dof_config = {
...
# 相机名到 prompt 中自然语言视角名的映射。输入文本 prompt 里不会直接写
# camera_front，而会写类似 "front view"、"right wrist view"。
_CAM_NAME_MAPPING = {
...
# 外部环境传进来的相机 key 到工程内部 view key 的映射。
...
# 环境 state 字段到动作语义字段的映射。当前文件主要用 follow1_pos /
# follow2_pos 这类字段构造 agent_pos。
action_key_mapping = {
...
# 单臂动作中位置、旋转、夹爪的切片定义。rpy 表示欧拉角格式，
# so3 表示 6D rotation 表示。
dim_dof_config = {
...
def euler_to_matrix_zyx_batch_nb(eulers):
    """把一批 ZYX 欧拉角 [roll, pitch, yaw] 转成旋转矩阵。"""
    N = eulers.shape[0]
...
def so3_to_euler_zyx_batch_nb(batch_so3):
    """把 6D rotation 批量转换为规范化后的 ZYX 欧拉角。"""
...
def move_to_cuda(obj, device=device):
    """递归地把 tensor/BatchFeature/dict/list/tuple 搬到指定设备。"""
...
def extract_components(data, config, is_rpy):
    """按动作语义切片提取 xyz、rot、gripper 等子向量。"""
...
class WallxInferArgs:
    ...
    保存 checkpoint、动作模式、action horizon、状态/动作表示方式等推理开关。
    """
...
class WallxModelWrapper:
    """RoboChallenge 推理的核心包装器。

    这个类负责加载 processor/model/normalizer，并把外部环境输入
    转成模型需要的 BatchFeature，最后把模型输出转回机器人动作格式。
    """
    ...
    def get_model_and_processor(self):
        """加载配置、processor、action tokenizer、normalizer 和模型权重。

        这里会把模型放到 GPU，并切换为 eval 模式；后续推理都复用
        self.model 和 self.processor。
        """
    ...
    def _register_normalizers(self):
        """加载或构造 action/proprioception 的归一化器。

        模型训练时通常在归一化空间里预测动作，所以推理前需要 normalize
        机器人状态，推理后需要 unnormalize 动作。
        """
    ...
    def get_text_ar(self, instruction, camera_names, norm_state=None, state_mask=None):
        """构造自回归动作 token 模式的多模态 prompt。

        prompt 中会插入每个相机视角的 <|image_pad|> 占位符，以及
        机器人状态的文本/特殊 token 表示。
        """
    ...
    def get_text_flow(
        ...
        """构造 diffusion/flow 动作模式的多模态 prompt。

        和 AR 模式相比，这里会在 assistant 后追加 action_horizon 个
        <|action|> token，模型会在这些位置上生成连续动作。
        """
        role_start_symbol = "<|im_start|>"
        role_end_symbol = "<|im_end|>"
        vision_start_symbol = "<|vision_start|>"
    ...
    def resize_images(self, observation):
        """从 observation 中取出相机图像并 resize 到 Qwen-VL 可接受尺寸。

        这一步仍在 CPU/PIL 上完成，是未来做异步 CPU 预处理队列时
        最自然的预处理阶段之一。
        """
        image_inputs = []
        view_candidates = [
    ...
    def _construct_input(
        ...
        """把 observation/instruction 拼成模型可直接使用的输入 batch。

        主要步骤：
        1. 从 observation 取 agent_pos、agent_pos_mask、dof_mask。
        2. 对 agent_pos 做 proprioception normalize。
        3. 根据 action mode 构造文本 prompt。
        4. resize 图像并调用 processor.image_processor 得到 pixel_values。
        5. 根据 image_grid_thw 展开 <|image_pad|>，再 tokenizer。
        6. 构造 moe_token_types、dataset_names，最后搬到 GPU。

        如果后续实现异步 CPU 预处理队列，应优先把本函数末尾的
        move_to_cuda 拆出去，让这个函数只返回 CPU tensors。
        """
        additional_inputs = {}
    ...
    def preprocess(self, state, views, valid_action_dim):
        """把外部机器人 state/views 整理成统一 observation 字典。

        state 里通常有 follow1_pos/follow2_pos；views 里是相机图像。
        这里会根据 valid_action_dim 构造 agent_pos、agent_pos_mask 和
        dof_mask，并把相机 key 映射为内部 view 名。
        """
    ...
    def model_output_process(self, action_pred, state):
        """把模型输出动作转换到最终控制需要的坐标/旋转表示。

        如果模型输出的是 delta action，会和当前 state 合成为绝对动作；
        如果输出 rotation 是 6D SO3，会转换为 RPY。
        """
    ...
    def postprocess(self, action_pred, interpolate_multiplier=None):
        """裁剪 action horizon，并序列化为控制端需要的 follow1/follow2 字典。"""
    ...
    def predict_action_rtc(
        ...
        当前实现是同步串行流程：
        preprocess -> _construct_input -> move_to_cuda -> 模型推理 -> 动作后处理。
        如果要改成异步 CPU 预处理队列，建议把该函数拆成：
        prepare_inference_inputs_cpu、run_inference_gpu、postprocess_action。
        """
    ...
    def generate_flow_action(
        ...
        """调用模型的 diffusion/flow 动作生成路径。

        这里已经进入 GPU 推理核心；耗时报告中的 total_time、ODE
        integration 等计时主要来自 model.generate_flow_action 内部。
        """
        model_action_dim = sum(self.dof_config.values())
    ...
    def generate_ar_action(self, inputs):
        """调用模型的自回归离散动作 token 生成路径，并解码成动作。"""
...
class WallxInfer:
    ...
    外部代码通常只需要创建 WallxInfer，然后调用 run_infer_robochallenge。
    """
    ...
    def run_infer_robochallenge(
        ...
        """外部调用入口：传入 state、views、instruction，返回机器人动作。"""
...
# 这个 main 块基本只是占位；当前类里没有 run_infer 方法，实际使用时
# 通常由其他脚本/服务实例化 WallxInfer 并调用 run_infer_robochallenge。
